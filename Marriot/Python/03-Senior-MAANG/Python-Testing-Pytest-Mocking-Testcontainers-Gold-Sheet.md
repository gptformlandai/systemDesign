...
| REST test (async) | `WebTestClient.get("/")` | `AsyncClient(app=app).get("/")` |
...
A: Two approaches: (1) `TestClient` from `fastapi.testclient` — wraps the ASGI app in a synchronous client using `anyio`. The test function is a normal `def` (not async). This covers most cases — it exercises the full async path internally but tests are written synchronously. (2) `httpx.AsyncClient(app=app, base_url="http://test")` with `@pytest.mark.asyncio` — the test function is `async def` and uses `await`. Required when testing WebSocket endpoints, streaming responses, or when you need to await other async fixtures alongside the request. Always clear `dependency_overrides` after the test to prevent state leakage between test modules.
...
- [ ] I use `TestClient` for sync tests; `httpx.AsyncClient` + `pytest-asyncio` for async tests
